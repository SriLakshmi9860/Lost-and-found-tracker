...
export default function Feed() {
  const user_info = JSON.parse(window.localStorage.getItem('user'));
  setConstraint(true);
  ...
  useEffect(() => {
    Axios.get("http://localhost:4000/items")
      .then((response) => {
        ...
        const myItems = allItems.filter(item => item.userId === user_info._id);
        setMaxPages(Math.ceil(myItems.length / itemsPerPage));

        const startIndex = (page - 1) * itemsPerPage;
        const data = myItems.slice(startIndex, startIndex + itemsPerPage);
